import React from "react";
import Footer from "../../components/Footer/Footer";
import Header from "../../components/Header";

const StructuresEnAcier = () => {
  return (
    <>
      <Header title="Structures En Acier" />
      <section class="bg-white mb-20 ">
        <div class="container px-6 py-10 mx-auto">
          <div class="grid grid-cols-1 gap-8 mt-8 xl:mt-12 xl:gap-16 md:grid-cols-2 xl:grid-cols-3">
            <div class="space-y-3">
              <span class="inline-block p-3 text-blue-500 bg-blue-100 rounded-full  ">
                <svg
                  xmlns="http://www.w3.org/2000/svg"
                  class="w-6 h-6"
                  fill="none"
                  viewBox="0 0 24 24"
                  stroke="currentColor"
                >
                  <path
                    stroke-linecap="round"
                    stroke-linejoin="round"
                    stroke-width="2"
                    d="M17.657 18.657A8 8 0 016.343 7.343S7 9 9 10c0-2 .5-5 2.986-7C14 5 16.09 5.777 17.656 7.343A7.975 7.975 0 0120 13a7.975 7.975 0 01-2.343 5.657z"
                  />
                  <path
                    stroke-linecap="round"
                    stroke-linejoin="round"
                    stroke-width="2"
                    d="M9.879 16.121A3 3 0 1012.015 11L11 14H9c0 .768.293 1.536.879 2.121z"
                  />
                </svg>
              </span>

              <h1 class="text-2xl font-semibold text-gray-700 capitalize ">
                Ingénierie
              </h1>

              <p class="text-gray-500 ">
                Conception architecturale et fonctionnelle interne complète par
                le logiciel 3D FE calcul statique de toutes les structures en
                acier d’une usine.
                <br />
                <br />
                Modèles disponibles Structures standard en acier, comme les
                ponts de convoyeur, les passerelles ouvertes ou fermées, les
                plates-formes, les échelles pour un fonctionnement sûr et
                facile, maintenance et service de l’usine.
                <br />
                <br />
                Structures en acier sur mesure, spécifiques au contrat, telles
                que les entablements et les cadres solides pour soutenir les
                machines de traitement ou les sections.
              </p>

              <a
                href={() => false}
                class="inline-flex items-center -mx-1 text-sm text-blue-500 capitalize transition-colors duration-300 transform  hover:underline hover:text-blue-600 "
              >
                <span class="mx-1">read more</span>
                <svg
                  class="w-4 h-4 mx-1 rtl:-scale-x-100"
                  fill="currentColor"
                  viewBox="0 0 20 20"
                  xmlns="http://www.w3.org/2000/svg"
                >
                  <path
                    fill-rule="evenodd"
                    d="M12.293 5.293a1 1 0 011.414 0l4 4a1 1 0 010 1.414l-4 4a1 1 0 01-1.414-1.414L14.586 11H3a1 1 0 110-2h11.586l-2.293-2.293a1 1 0 010-1.414z"
                    clip-rule="evenodd"
                  ></path>
                </svg>
              </a>
            </div>

            <div class="space-y-3">
              <span class="inline-block p-3 text-blue-500 bg-blue-100 rounded-full  ">
                <svg
                  xmlns="http://www.w3.org/2000/svg"
                  class="w-6 h-6"
                  fill="none"
                  viewBox="0 0 24 24"
                  stroke="currentColor"
                >
                  <path
                    stroke-linecap="round"
                    stroke-linejoin="round"
                    stroke-width="2"
                    d="M11 4a2 2 0 114 0v1a1 1 0 001 1h3a1 1 0 011 1v3a1 1 0 01-1 1h-1a2 2 0 100 4h1a1 1 0 011 1v3a1 1 0 01-1 1h-3a1 1 0 01-1-1v-1a2 2 0 10-4 0v1a1 1 0 01-1 1H7a1 1 0 01-1-1v-3a1 1 0 00-1-1H4a2 2 0 110-4h1a1 1 0 001-1V7a1 1 0 011-1h3a1 1 0 001-1V4z"
                  />
                </svg>
              </span>

              <h1 class="text-2xl font-semibold text-gray-700 capitalize ">
                Tour de machinerie
              </h1>

              <p class="text-gray-500 ">
                Tours de machinerie à contrat, ou hangar, conçues pour
                l’installation d’une installation complète, y compris le soutien
                à plusieurs étages des machines de traitement, des blocs de
                silo, des escaliers, des couvertures et des revêtements.
              </p>

              <a
                href={() => false}
                class="inline-flex items-center -mx-1 text-sm text-blue-500 capitalize transition-colors duration-300 transform  hover:underline hover:text-blue-600 "
              >
                <span class="mx-1">read more</span>
                <svg
                  class="w-4 h-4 mx-1 rtl:-scale-x-100"
                  fill="currentColor"
                  viewBox="0 0 20 20"
                  xmlns="http://www.w3.org/2000/svg"
                >
                  <path
                    fill-rule="evenodd"
                    d="M12.293 5.293a1 1 0 011.414 0l4 4a1 1 0 010 1.414l-4 4a1 1 0 01-1.414-1.414L14.586 11H3a1 1 0 110-2h11.586l-2.293-2.293a1 1 0 010-1.414z"
                    clip-rule="evenodd"
                  ></path>
                </svg>
              </a>
            </div>

            <div class="space-y-3">
              <span class="inline-block p-3 text-blue-500 bg-blue-100 rounded-full  ">
                <svg
                  xmlns="http://www.w3.org/2000/svg"
                  class="w-6 h-6"
                  fill="none"
                  viewBox="0 0 24 24"
                  stroke="currentColor"
                >
                  <path
                    stroke-linecap="round"
                    stroke-linejoin="round"
                    stroke-width="2"
                    d="M5 3v4M3 5h4M6 17v4m-2-2h4m5-16l2.286 6.857L21 12l-5.714 2.143L13 21l-2.286-6.857L5 12l5.714-2.143L13 3z"
                  />
                </svg>
              </span>

              <h1 class="text-2xl font-semibold text-gray-700 capitalize ">
                Filtres de localisation et séparateurs magnétiques de dégivrage
              </h1>

              <p class="text-gray-500 ">
                Filtre à jet miniature pour aspiration et filtration immédiate
                de la poussière, dans les systèmes de transport horizontal et
                vertical avec nettoyage automatique séparé des sacs filtrants.
                <br />
                <br />
                Boîtes compactes avec plaques magnétiques et tiges pour la
                séparation des particules étrangères ferromagnétiques dans les
                produits secs et volumineux à écoulement libre.
              </p>

              <a
                href={() => false}
                class="inline-flex items-center -mx-1 text-sm text-blue-500 capitalize transition-colors duration-300 transform  hover:underline hover:text-blue-600 "
              >
                <span class="mx-1">read more</span>
                <svg
                  class="w-4 h-4 mx-1 rtl:-scale-x-100"
                  fill="currentColor"
                  viewBox="0 0 20 20"
                  xmlns="http://www.w3.org/2000/svg"
                >
                  <path
                    fill-rule="evenodd"
                    d="M12.293 5.293a1 1 0 011.414 0l4 4a1 1 0 010 1.414l-4 4a1 1 0 01-1.414-1.414L14.586 11H3a1 1 0 110-2h11.586l-2.293-2.293a1 1 0 010-1.414z"
                    clip-rule="evenodd"
                  ></path>
                </svg>
              </a>
            </div>
          </div>
        </div>
      </section>
      <Footer />
    </>
  );
};

export default StructuresEnAcier;
